...
- Require `data/library.json`, enough tops and bottoms for the requested count, and at least one local identity reference at `data/model-reference.png` or `data/model-references/` (or `WARDROBE_MODEL_REFERENCE`).
...
1. Select one identity reference for the outfit. If `data/model-references/` contains multiple photos, rotate references across the collection and record the selected path in the outfit record. Use the primary `data/model-reference.png` as the fallback.
2. Exact top cutout
